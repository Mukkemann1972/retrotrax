...
void HelpPanel::rebuild()
{
    ...
    topics.add ({ loc::t ("Effekt-Spalte", "Effect column"),
        loc::t (
            ...
            "  5xy  Tone-Portamento laeuft weiter + Lautstaerke-Slide\n"
            "       (x hoch, y runter) - Melodie gleitet und blendet zugleich\n"
            "  6xy  Vibrato laeuft weiter + Lautstaerke-Slide (x hoch, y runter)\n"
            "  7xy  Tremolo - Lautstaerke wobbelt: x = Tempo, y = Tiefe\n"
            "  8xx  Panning - 00 = links, 80 = Mitte, FF = rechts (Stereo)\n"
            ...
            "  E00  LED-Filter aus - Summe wieder hell/transparent\n"
            "  E01  LED-Filter an - dumpfer Amiga-Tiefpass auf der ganzen Summe\n"
            "       (wie die LED-Taste am echten Amiga, wirkt auf alle Spuren)\n"
            "  E1x  Fine Porta hoch - Tonhoehe einmal pro Zeile leicht anheben\n"
            "  E2x  Fine Porta runter - einmal pro Zeile leicht absenken\n"
            "  EAx  Fine Volume-Slide hoch - Lautstaerke einmal um x anheben\n"
            "  EBx  Fine Volume-Slide runter - einmal um x absenken\n"
            "  E60  Pattern-Loop-Marke - ab hier merkt sich das Pattern die\n"
            "       Sprungstelle fuer E6x\n"
            "  E6x  Pattern-Loop - springt x-mal zurueck zur E60-Marke (Fill\n"
            "       oder Break x-fach wiederholen)\n"
            ...
            "       Akkorde)\n"
            "  EEx  Pattern-Delay - diese Zeile x Tick-Zyklen laenger halten\n"
            "       (Zeile \"anhalten\", ohne sie neu anzuschlagen)\n"
            ...
            "  5xy  continue tone portamento + volume slide (x up, y down)\n"
            "       - the melody glides and fades at the same time\n"
            "  6xy  continue vibrato + volume slide (x up, y down)\n"
            "  7xy  tremolo - volume wobble: x = speed, y = depth\n"
            "  8xx  panning - 00 = left, 80 = center, FF = right (stereo)\n"
            ...
            "  E00  LED filter off - mix back to bright/transparent\n"
            "  E01  LED filter on - muffled Amiga lowpass on the whole mix\n"
            "       (like the LED button on a real Amiga, affects every track)\n"
            "  E1x  fine porta up - nudge pitch up once per row\n"
            "  E2x  fine porta down - nudge pitch down once per row\n"
            "  EAx  fine volume slide up - raise volume by x once\n"
            "  EBx  fine volume slide down - lower volume by x once\n"
            "  E60  pattern loop marker - the pattern remembers this row as\n"
            "       the jump-back point for E6x\n"
            "  E6x  pattern loop - jump back to the E60 marker x times (repeat\n"
            "       a fill or break x times)\n"
            ...
            "       staggered chords)\n"
            "  EEx  pattern delay - hold this row for x extra tick cycles\n"
            "       (pause the row without re-triggering it)\n"
    ...
    topics.add ({ loc::t ("Sprachsynthese", "Speech synthesis"),
        loc::t (
            "Jeder Slot kann auch eine Robo-Stimme sein - im Geist der alten\n"
            "Sprachchips (Speak&Spell, C64-SAM, Amiga-Narrator), aber komplett\n"
            "selbst gebaut (Formant-Synthese, keine fremde Bibliothek). Das\n"
            "Ergebnis ist ein ganz normales Sample: der Akai-Filter (FX-Knopf,\n"
            "12-Bit/Drive/Loop) wirkt danach ganz normal mit.\n\n"
            "So geht's: Slot oben waehlen, dann den Knopf SPRACH druecken. Der\n"
            "Slot wird zu einem Sprach-Instrument (Start-Satz auf Englisch),\n"
            "das Sprach-Fenster geht auf.\n\n"
            "Im Sprach-Fenster stellst du ein:\n"
            "- CHARAKTER: SAM (C64) - schrill, blechern, hektisch. NARRATOR\n"
            "  (AMIGA) - weicher, dumpfer, ruhiger. Beide nutzen denselben\n"
            "  Klangmotor, nur andere Grundwerte.\n"
            "- TEXT: der Satz, der gesprochen wird (einfaches Englisch reicht -\n"
            "  keine perfekte Aussprache, das gehoert zum Retro-Charme).\n"
            "- SPEED (50-200%): Sprechtempo, langsamer/schneller.\n"
            "- PITCH (60-260 Hz): Grundtonhoehe der Stimme.\n"
            "- STIMMLAGE (-100 bis +100%): verschiebt die Formanten - hoch/hell\n"
            "  bis tief/gross.\n"
            "- MUNDOEFFNUNG (0-100%): eng/genuschelt bis weit offen.\n\n"
            "SPRECHEN rendert Text + Regler frisch (noetig nach jeder Aenderung),\n"
            "TEST spielt das aktuelle Ergebnis als Probe-Note ab. Die Sprach-\n"
            "Parameter werden mit dem Song gespeichert - beim naechsten Oeffnen\n"
            "des Slots kannst du den Text weiter anpassen und neu rendern.",
            "Every slot can also be a robot voice - in the spirit of the old\n"
            "speech chips (Speak&Spell, C64 SAM, Amiga Narrator), but completely\n"
            "self-built (formant synthesis, no outside library). The result is a\n"
            "perfectly normal sample: the Akai filter (FX button, 12-bit/drive/\n"
            "loop) applies to it just like any other sample.\n\n"
            "How: pick a slot at the top, then press the SPEECH button. The slot\n"
            "becomes a speech instrument (starting sentence in English), and the\n"
            "speech window opens.\n\n"
            "In the speech window you set:\n"
            "- CHARACTER: SAM (C64) - shrill, tinny, hectic. NARRATOR (AMIGA) -\n"
            "  softer, duller, calmer. Both use the same sound engine, just\n"
            "  different starting values.\n"
            "- TEXT: the sentence to be spoken (simple English is enough - not\n"
            "  perfect pronunciation, that's part of the retro charm).\n"
            "- SPEED (50-200%): speaking tempo, slower/faster.\n"
            "- PITCH (60-260 Hz): the voice's base pitch.\n"
            "- THROAT (-100 to +100%): shifts the formants - high/bright to\n"
            "  low/large.\n"
            "- MOUTH (0-100%): narrow/mumbled to wide open.\n\n"
            "SPEAK renders text + controls freshly (needed after every change),\n"
            "TEST plays the current result as a preview note. The speech\n"
            "parameters are saved with the song - next time you open the slot\n"
            "you can keep tweaking the text and render again.") });
    ...
    topics.add ({ loc::t ("Song speichern & oeffnen", "Save & open song"),
        loc::t (
            ...
            "RTX (gepackt): Tippst du beim Speichern eine Endung .rtx, kommt eine\n"
            "gepackte Fassung heraus - dieselbe Musik, aber viel kleiner (beim\n"
            "Demo-Song 15 statt 43 KB). Gedacht zum MITSCHICKEN und Einbauen: der\n"
            "Web-Player und der kleine Replayer lesen sie direkt. Die Samples\n"
            "stecken mit drin, es klingt Ton fuer Ton gleich. Zum WEITERARBEITEN\n"
            "nimmst du weiter .retrotrax - im gepackten .rtx sind die Drum-Pads\n"
            "nicht enthalten (die spielen im Sequencer ohnehin nicht mit).\n\n"
            ...
            "(once each).\n\n"
            "RTX (packed): type a .rtx extension when saving and you get a packed\n"
            "version - same music, much smaller (15 instead of 43 KB for the demo\n"
            "song). Made for SHARING and embedding: the web player and the small\n"
            "replayer read it directly. The samples are inside, it sounds note for\n"
            "note the same. To keep EDITING, stay with .retrotrax - the packed .rtx\n"
            "does not carry the drum pads (which the sequencer does not play anyway).\n\n"
    ...
    topics.add ({ loc::t ("Drum-Kit (16 Pads)", "Drum kit (16 pads)"),
        loc::t (
            ...
            "- Bei eingeschaltetem REC landet dieser echte Anschlag auch im Pattern -\n"
            "  live beim Abspielen oder Schritt fuer Schritt im Stopp, genau wie die\n"
            "  Tastatur-Drum-Eingabe, nur mit echter statt zufaelliger Velocity.\n"
            ...
            "- With REC switched on, that real hit also lands in the pattern - live\n"
            "  while playing or step by step while stopped, just like the keyboard\n"
            "  drum input, but with real velocity instead of random variation.\n"
    ...
    topics.add ({ loc::t ("FX: Master-Effekte (ganze Kette)", "FX: master effects (full chain)"),
        loc::t (
            ...
            "zuletzt ZUSAMMENGEHALTEN.\n\n"
            "ZERRE (Distortion) - formt:\n"
            "  STAERKE = wie hart angefahren wird. Es ist eine weiche Saettigung wie\n"
            "  bei den alten Geraeten, kein hartes Abschneiden: es wird dichter und\n"
            "  druckvoller, nicht kratzig. MIX 0 = aus.\n\n"
            "FLANGER - bewegt:\n"
            "  Der klassische \"Duesenjet\": eine ganz kurze Verzoegerung faehrt langsam\n"
            "  hin und her. TEMPO = wie schnell, TIEFE = wie weit, MIX 0 = aus.\n"
            "  Links und rechts laufen versetzt - das macht ihn schoen breit.\n\n"
            "PHASER - bewegt, aber weicher:\n"
            "  Verwandt mit dem Flanger, klingt aber nach \"Wusch\" statt nach Metall.\n"
            "  TEMPO / TIEFE wie oben, MIX 0 = aus.\n\n"
            "ECHO (Delay) - Raum:\n"
            "  ZEIT = Verzoegerung, RUECKKOPPLUNG = wie oft es wiederholt,\n"
            "  MIX = wie laut das Echo. MIX 0 = aus.\n\n"
            "HALL (Reverb) - Raum:\n"
            "  RAUM = Groesse des Raums, MIX = wie viel Hall. MIX 0 = aus.\n\n"
            "EQ (3-Band) - Ton:\n"
            "  BASS / MITTEN / HOEHEN anheben oder absenken (+/- dB, 0 = flach) -\n"
            "  gibt dem Mix Druck oder nimmt Schaerfe raus.\n\n"
            "KOMPRESSOR - haelt zusammen:\n"
            "  Bremst, was lauter wird als die SCHWELLE; das VERHAELTNIS sagt wie\n"
            "  stark (4:1 ist ein guter Anfang). Danach wird wieder angehoben, damit\n"
            "  es dichter klingt und nicht leiser. Tipp: MIX unter 100 % laesst das\n"
            "  trockene Signal daneben stehen - der Mix wird dicht, die Anschlaege\n"
            "  bleiben aber lebendig. MIX 0 = aus.\n\n"
            ...
            "DISTORTION - shapes:\n"
            "  DRIVE = how hard it is driven. It is a soft saturation like the old\n"
            "  gear, not hard clipping: it gets denser and punchier, not scratchy.\n"
            "  MIX 0 = off.\n\n"
            "FLANGER - moves:\n"
            "  The classic \"jet plane\": a very short delay slowly sweeping back and\n"
            "  forth. RATE = how fast, DEPTH = how far, MIX 0 = off. Left and right\n"
            "  run offset from each other - that makes it nice and wide.\n\n"
            "PHASER - moves, but softer:\n"
            "  Related to the flanger, but sounds like a \"whoosh\" instead of metal.\n"
            "  RATE / DEPTH as above, MIX 0 = off.\n\n"
            "ECHO (delay) - room:\n"
            "  TIME = delay length, FEEDBACK = how often it repeats, MIX = how loud\n"
            "  the echo. MIX 0 = off.\n\n"
            "REVERB - room:\n"
            "  ROOM = room size, MIX = how much reverb. MIX 0 = off.\n\n"
            "EQ (3-band) - tone:\n"
            "  Boost or cut LOW / MID / HIGH (+/- dB, 0 = flat) - gives the mix punch\n"
            "  or takes the harshness out.\n\n"
            "COMPRESSOR - holds it together:\n"
            "  Holds back whatever gets louder than the THRESHOLD; the RATIO says how\n"
            "  strongly (4:1 is a good start). Afterwards it lifts again, so it sounds\n"
            "  denser rather than quieter. Tip: MIX below 100 % keeps the dry signal\n"
            "  alongside - the mix gets dense while the hits stay alive. MIX 0 = off.\n\n"
            "ALL of them sit at MIX 0 = off, so your sound stays unchanged until you\n"
            "turn something up. The settings are saved with the song (.retrotrax).") });
